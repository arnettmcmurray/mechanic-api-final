# === seed.py — safe manual seeding (no auto-wipe) ===
import os
...
def seed_data():
    """Populate DB only if empty; keeps all data permanent."""
    with app.app_context():
        ...
        # === Service Tickets ===
        tickets = [
            ServiceTicket(description="Brake pad replacement", status="In Progress",
                          customer_id=1, mechanics=[mechanics[1]]),
            ServiceTicket(description="Oil change", status="Closed",
                          customer_id=2, mechanics=[mechanics[2]]),
            ServiceTicket(description="Engine diagnostics", status="Open",
                          customer_id=3, mechanics=[mechanics[3]]),
            ServiceTicket(description="Replace alternator", status="In Progress",
                          customer_id=4, mechanics=[mechanics[6]]),
            ServiceTicket(description="Install new timing belt", status="Open",
                          customer_id=5, mechanics=[mechanics[6], mechanics[2]]),
            ServiceTicket(description="Battery replacement", status="Closed",
                          customer_id=6, mechanics=[mechanics[5]]),
            ServiceTicket(description="Suspension inspection", status="In Progress",
                          customer_id=7, mechanics=[mechanics[4], mechanics[1]]),
            ServiceTicket(description="Body work estimate", status="Open",
                          customer_id=10, mechanics=[mechanics[7], mechanics[0]]),
        ]
